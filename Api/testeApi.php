...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'PUT':
        ...
        $sql = "UPDATE cliente SET nome='$nome', site='$site', email='$email', rg='$rg' WHERE idcli=$id";
    ...
    case 'POST':
        ...
        $sql = "INSERT INTO cliente (nome, site, email,rg) VALUES ('$nome','$site', '$email','$rg')";
        if ($conn->query($sql) === TRUE) {
            if ($conn->affected_rows > 0) {
    ...
    case 'GET':
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $response[] = ['id' => $row['idcli'], 'nome' => $row['nome'], 'site' => $row['site'], 'rg' => $row['rg'], 'email' => $row['email']];
            }
        } else {
